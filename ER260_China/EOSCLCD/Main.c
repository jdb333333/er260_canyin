...
{
	//jdb2018-10-24餐饮结账保存参数
	BYTE tmpSaveAppl_EntryCounter;
	BCD tmpSaveEntry;
	WORD tmpSaveCode;
    ...
    else if (ApplVar.Key.Code < TEND + 100)   /* Tendering Functions 900 - 999 */
    {
    	...
    	if(ApplVar.FPb){
			tmpSaveAppl_EntryCounter = Appl_EntryCounter;
			tmpSaveEntry = ApplVar.Entry;
			tmpSaveCode = ApplVar.Key.Code;
			tmpSaveNumberEntry = ApplVar.NumberEntry;
			tmpSaveSubTotal = ApplVar.SubTotal;
			tmpSaveSaleAmt = ApplVar.SaleAmt;
			tmpSaveAmt = ApplVar.Amt;
			
			if (!WaitForYesNo("CHECKOUT",true))
     			return;
			PbPrint();
			PbTender();

			Appl_EntryCounter = tmpSaveAppl_EntryCounter;
			ApplVar.Entry = tmpSaveEntry;
			ApplVar.Key.Code = tmpSaveCode;
			ApplVar.NumberEntry = tmpSaveNumberEntry;
			ApplVar.SubTotal = tmpSaveSubTotal;
			ApplVar.SaleAmt = tmpSaveSaleAmt;
			ApplVar.Amt = tmpSaveAmt;
    	}
        Tender();
#if (SCREENLN>6)
        DispStrXY(FormatQtyStr(Msg[XIANGSHU].str, &ApplVar.SaleQty, SCREENWD),0,SCREENLN-2);//ccr2016-03-03
...
{
//jdb2018-10-23餐饮功能只有PRINT PB打印
#if 1
if(!TESTBIT(ApplVar.PrintFlag, BIT0)){
#if (defined(CASE_EJSD) || defined(CASE_EJFLASH))
