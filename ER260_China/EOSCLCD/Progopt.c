...
//jdb2018-10-25
void strTo_Low4_Bin(BYTE type,BYTE *byte)
{
    short i;
    BYTE sByte;
...
//jdb2018-10-25
void strTo_High4_Bin(BYTE type,BYTE *byte)
{
    short i;
    BYTE sByte;
...
//lyq modify 2003\10\22 end
        ...
        break;
#if 1//jdb2018-10-25编辑TAX1 TAX2
    case 46://get DEPT TAX1
        if (Appl_EntryCounter)
            strTo_Low4_Bin(type,(BYTE *)opt);
        DispStrXY("TAX", 0,0xff);
        sP = 0;
        for (sLp = 0; sLp < 4; sLp++)
        {
        ...
        break;
	case 47://get DEPT TAX2
        if (Appl_EntryCounter)
            strTo_High4_Bin(type,(BYTE *)opt);
        DispStrXY("TAXGST", 0,0xff);
        sP = 0;
        for (sLp = 4; sLp < 8; sLp++)
        {
            if (*((BYTE *)opt) & (0x01 << sLp))
            {
                ProgLineMes[sP] = (sLp - 4) + 0x31;
                sP++;
            }
        }
        break;
