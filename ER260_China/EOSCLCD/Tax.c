...
#if 1//jdb2018-10-25部类关联两种税
void AddTaxItem(BYTE tax)
{
#if 1//jdb2018-10-26税A关联税A_1 A_2。。。
    BYTE t, i, k, taxcount;
	BYTE taxnum, relation;
	...
	if(taxnum > 0){
		...
		if(relation > 0){
			...
			for (k = 0; k < 4; k++)
			{
				if (relation & t)
				{
					Add(&ApplVar.TaxItem[k * 2 + 4], &ApplVar.Amt);
					Add(&ApplVar.TaxItem[k * 2 + 5], &ApplVar.Amt);
					break;
				}
				t <<= 1;
			}
	        ...
	        for (ApplVar.TaxNumber = 0; ApplVar.TaxNumber < ApplVar.AP.Tax.Number; ApplVar.TaxNumber++)
	        {
		        ...
		        if (CheckNotZero(&ApplVar.Amt))
		        {
		            ...
		            if (!TESTBIT(ApplVar.Tax.Options, BIT0)) /* add on ? */
		            {
						...
						if(ApplVar.TaxNumber >= 4)//jdb2018-10-26关联税不加入SUBTOTAL
						{
						}
						else
